...
public class ExportXML implements IExport
{
    ...
    public String exportParts(File file, CampaignGUI gui) {
        ...
        if (gui.getTab(GuiTabType.WAREHOUSE) != null) {
            try {
                ...
                if (row < 0) {
                    MekHQ.getLogger().log(IExport.class, METHOD_NAME, LogLevel.WARNING,
                ...
                MekHQ.getLogger().log(IExport.class, METHOD_NAME, LogLevel.INFO,
                        "Parts saved to " + file); //$NON-NLS-1$
                report = Integer.toString(rows.length) + " " + resourceMap.getString("dlgPartsSavedFile.text");
            } catch (Exception ex) {
                MekHQ.getLogger().error(IExport.class, METHOD_NAME, ex);
    ...
    public String exportCampaignOptions(File file, Campaign campaign, CampaignOptions co) {
        ...
        try {
            ...
            MekHQ.getLogger().error(IExport.class, METHOD_NAME, ex); //$NON-NLS-1$
            report = resourceMap.getString("dlgProblemWritingFile.text");
        }
